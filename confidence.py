...
def main():
    ...
    for backdoor_count in backdoor_count_list:
        ...
        full_kk = []
        for i in range(10):
            data = np.reshape(adv_9[i], (1, 32, 32, 3))
            ...
            if i == 0:
                ...
                full_kk = np.vstack((full_kk, conf))
        ...
        print(np.round(np.min(dist_list),3))
        print(np.round(np.max(dist_list),3))
        print(np.round(np.mean(dist_list),3))
        print(np.round(np.std(dist_list),3))
